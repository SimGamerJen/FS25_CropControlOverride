...
| Existing Field Cleanup | Whether existing non-compliant NPC fields of that crop appear in validation and may be reset by the cleanup tools. |
...
Controls whether existing NPC fields of this crop are included in validation and may be reset by CCO cleanup tools.

If this is `ON`, a rule change that makes existing NPC fields non-compliant is guarded by APPLY / FORCE APPLY. After FORCE APPLY, those fields appear in VALIDATION and are eligible for the dry-run / confirm cleanup workflow.

If this is `OFF`, existing fields are deliberately grandfathered: they are excluded from validation and are not reset by the cleanup workflow. This option does not reset any field merely by clicking APPLY.
...
↓
Server validates the proposed rule against existing NPC fields
↓
If blocked fields are found, the client keeps the staged rule and offers FORCE APPLY
↓
After a safe or deliberately forced apply, the server updates its active savegame XML
↓
...
On a dedicated server, the server is authoritative for the validation totals, blocked-field details, dry-run result, and confirmed cleanup. Remote clients display the server-synchronised validation inventory rather than attempting to reconstruct it from incomplete local field data.
...
7. If the server blocks the change, confirm the staged values remain visible and the button changes to `FORCE APPLY`.
8. Use `FORCE APPLY` only if the reported existing fields are intended to enter validation and cleanup.
9. Confirm the server-side CCO savegame XML updates.
10. Confirm no local client-side CCO `savegame?.xml` file has been created.
...
- Synchronises the server-authoritative validation summary and blocked NPC field inventory to dedicated-server clients.
- Fixes forced rules saving successfully on the server while the remote Validation tab incorrectly remains empty.
- Routes blocked-field dry-runs and confirmed cleanup actions through the dedicated server and returns the authoritative results to the requesting admin.
- Adds a delayed validation refresh after asynchronous field cleanup so clients receive the settled server field state.
- Corrects the missing apply-blocked localization reference and adds defensive fallback handling for missing l10n entries.
- Updates English, German, and French localization for the new dedicated-server waiting states.

### 2.1.0.0 Alpha 10.1

- Fixed dedicated-server rule rejections appearing as an unexplained settings resync.
- Remote admins now retain the staged rule when server validation blocks APPLY.
- The client now changes APPLY to FORCE APPLY after the server reports existing non-compliant NPC fields.
- A rejected APPLY no longer refreshes the unchanged ruleset or discards the proposed values.
- Renamed `Reset NPC Fields` to `Existing Field Cleanup` and clarified that OFF grandfathers existing fields by excluding them from validation and cleanup.
- Preserves server-authoritative XML ownership, admin validation, per-save compatibility, and Alpha 10 NPC-map regeneration behaviour.
...
Editing or publishing this mod is prohibited without written consent from the mod maker.
