...
function Calendar:previewShift(cropName, shift)
    ...
    local preview = {
        ...
        current = currentRow, default = defaultRow, proposed = proposedRow, proposedData = proposedData, impact = impact,
...
function Calendar:buildPreviewText(preview)
    ...
    local reconciliation = preview.reconciliation or {}
    local resultLine = preview.applyAllowed
        and ("APPLY ELIGIBLE. NPC reconciliation: keep=%d remap=%d review=%d; player fields=%d unchanged; %d available contract(s) will be rebuilt."):format(
            tonumber(reconciliation.keep or 0), tonumber(reconciliation.remap or 0), tonumber(reconciliation.review or 0),
            tonumber(reconciliation.playerSkipped or impact.playerFields or 0), boardAvailable)
    ...
    lines[#lines + 1] = ("Reconciliation: keep=%d | NPC remap=%d | review/unsafe=%d | player unchanged=%d"):format(
        tonumber(reconciliation.keep or 0), tonumber(reconciliation.remap or 0), tonumber(reconciliation.review or 0),
        tonumber(reconciliation.playerSkipped or impact.playerFields or 0))
